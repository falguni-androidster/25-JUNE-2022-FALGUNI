...
class AddtaskActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newlist.setOnClickListener {
            ...
            dialog.setContentView(R.layout.newadd_task)
                ...
                if (eddate.text.toString() != ""){
                   if (edtime.text.toString() != ""){
                       if (!status){
                           ...
                           var myModel = MyModel(id!!,edtask.text.toString(),eddate.text.toString(),edtime.text.toString(),sprepeat.selectedItem.toString(),sptask.selectedItem.toString())
                           ...
                           var insertID = myDatabase.insert(MyModel(it.id!!,edtask.text.toString(),eddate.text.toString(),edtime.text.toString(),sprepeat.selectedItem.toString(),sptask.selectedItem.toString()))
                           if (insertID > 0){
                               var intent = Intent(this,MainActivity::class.java)
                               finish()
                               startActivity(intent)
