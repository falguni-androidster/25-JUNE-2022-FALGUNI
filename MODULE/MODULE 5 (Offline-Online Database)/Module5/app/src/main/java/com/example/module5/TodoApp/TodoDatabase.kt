...
class TodoDatabase(var context : Context) : SQLiteOpenHelper(context,"mydbtask",null, VERSION_NO) {
    ...
    override fun onCreate(p0: SQLiteDatabase?) {
        val create_db =
            " CREATE TABLE $TABLE_NAME ( $ID integer primary key AUTOINCREMENT , $TASK VARCAR(20), $DATE VARCAR(20), $TIME VARCAR(20), $REPEAT VARCAR(20), $LIST VARCAR(20) )"
    ...
    fun insert(myModel: TodoModel) : Long {
    ...
    @SuppressLint("Range")
    fun getAll_Data() : MutableList<TodoModel>{
        val userlist : MutableList<TodoModel> = ArrayList()
        ...
        if(cursor.count > 0){
            if (cursor.moveToFirst()){
                do {
                    ...
                    val user_data = TodoModel(id,task,date,time,repeat,list)
    ...
    fun update_data(myModel: TodoModel) : Int {
        val db = this.writableDatabase
    ...
    fun delete_data(myModel: TodoModel) : Int{
        val db = this.writableDatabase
